...
const Home = () => {
  ...
  useEffect(() => {
    document.title = authState.isLoggedIn ? `${authState.user.name}'s tasks` : "Task Buddy";
  }, [authState]);
  ...
  return (
    <>
      <MainLayout>
        {!isLoggedIn ? (
          <div className='bg-purple-600 text-white h-[40vh] py-8 text-center'>
            <h1 className='text-2xl'> Welcome to Task Buddy</h1>
            <Link to="/signup" className='mt-10 text-xl block space-x-2 hover:space-x-4'>
              <span className='transition-[margin]'>Sign Up to Continue</span>
