...
const Navbar = () => {
  ...
  return (
    <>
      <header className='flex justify-between sticky top-0 p-4 bg-violet-400 text-white shadow-sm items-center'>
        <h2 className='cursor-pointer uppercase font-medium'>
          <Link to="/"> Task Buddy </Link>
        </h2>
        <ul className='hidden md:flex gap-4 uppercase font-medium'>
          {authState.isLoggedIn ? (
            <>
              ...
              <li className='py-2 px-3 cursor-pointer hover:bg-gray-500 transition rounded-sm' onClick={handleLogoutClick}>Logout</li>
            </>
          ) : (
            <li className='py-2 px-3 cursor-pointer hover:bg-gray-500 transition rounded-sm'><Link to="/login">Login</Link></li>
          )}
        </ul>
        ...
        <div className={`absolute md:hidden right-0 top-0 bottom-0 transition ${(isNavbarOpen === true) ? 'translate-x-0' : 'translate-x-full'} bg-gray-600 shadow-md w-screen sm:w-9/12 h-screen`}>
          <div className='flex'>
